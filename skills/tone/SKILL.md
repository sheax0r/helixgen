...
- The `helixgen` CLI is installed (the `setup` skill provisions it:
  `uv tool install 'helixgen[device]==0.38.0'` — isolated env, `helixgen`
...
once with `uv tool install --force 'helixgen[device,analyze]==0.38.0'`.
...
`uv tool install --force 'helixgen[device,analyze,capture]==0.38.0'`.
