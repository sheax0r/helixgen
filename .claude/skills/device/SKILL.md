...
committed** (`devices/` is gitignored), and rebuilt wholesale by every `device
sync`, so losing that file costs nothing (and the first sync after a v2→v3
...
and round-tripped through sync rebuilds. **"On the device" ⟺ the tone has a
slot.** There is
