...
- `helixgen patch <preset.hsp> <ops.json|-> [--json]` — apply a JSON **list** of ops (`set_param`, `set_enabled`, `add_block`, `remove_block`, `swap_model`, `set_input`) to the `.hsp` in one atomic invocation: all ops are applied in memory and the file is written once at the end, so an invalid op anywhere in the list leaves the file untouched. `-` reads the ops from stdin. `set_input` re-jacks a whole path's input endpoint — `{"op": "set_input", "path": 1, "jack": "mic"}`, where `jack` is `inst1`/`inst2`/`both`/`none`/`mic` — dropping params the target jack does not have (the mic has `LowCut` and no `Pad`). Preferred over repeated single-op verbs for multi-edit sessions.
