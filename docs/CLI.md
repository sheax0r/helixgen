...
- `helixgen library import <file.hsp|dir> [--artist --song | --descriptor]
  ...
  **copies** instead. A sibling `.md` (same stem) is folded into
  ...
  recovery command (copy the `.hsp` into the library directory).
...
`helixgen`) — the same names `device reorder`/`device copy` take.
...
- `helixgen device delete-ir <name-or-hash> [--yes] [--force-wedge]` — delete one user IR from the device **completely**: the registry entry (`/RemoveContent` on `-11`) plus its backing `.wav` (the device only garbage-collects the file lazily, which makes a quick re-import think it's "already on device"; removing the file closes that window). Presets that referenced it show a silent cab until it's re-imported. `--force-wedge` (32-hex hash only) additionally cleans the *wedged* state a delete→quick-re-import can leave (file + path index resolving, no registry entry) — never use it on a just-imported IR, whose listing may merely be lagging (a helixgen `push-ir` import is nudged into the listing, so only imports by other clients lag). Alternatively, re-running `device push-ir` on the same WAV distinguishes a stale listing cache from a wedge and heals the wedge by re-import — and since 0.32.0 the auto-upload paths (`install --auto-irs`, `device copy`) run the same wedge check and self-heal it too (#93). All of these need a `-11` listing with a row to nudge; an empty or failed listing keeps the trusting "already on device" answer, so `--force-wedge` is the sure clear.
...
- `helixgen device setlist list [<setlist>] [--json]` — **list the DEVICE's setlists and the presets they reference, in order**. Membership and order are device state (2026-09-09 file-copy design) — there is no local manifest to consult and nothing to keep in agreement. Read-only.
...
### The file-copy model (copy / rm / move)

Getting a tone onto the device is one call. There is no membership step, no registration, and no reconcile: the **device** is the truth about what is loaded and in what order, and the **library directory** (`$HELIXGEN_HOME/library/tones/*.hsp`) is the index of what you have authored. A tone's identity is the `meta.name` inside its `.hsp`.

- `helixgen device copy <tone.hsp> [--to <setlist>] [--pos N] [--no-irs] [--cid N] [--json]` — **copy an authored `.hsp` onto the device, upserting by `meta.name`**. If a preset of that name is already in the target, its content is **updated in place** via the non-activating existing-cid content write (the player's live tone is undisturbed); otherwise the tone is transcoded into the pool and a reference is added at `--pos` (append when omitted). So **re-copying an edited `.hsp` is how you push a revision** — there is no separate update verb and no hash bookkeeping. Transcodes template-free at full fidelity (dual-amp, parallel splits, snapshots, footswitch/EXP). **IRs upload by default**; `--no-irs` opts out — the reverse of the retired `install --auto-irs`, because a copy that leaves a cab reading "No Model" is a broken copy. Omit `--to` for a pool-only copy. An **occupied `--pos` is refused, not insert-shifted** (backlog #69: two references stacked at one position is uncataloged device behavior) — copy, then `device move`. Locks `library` (+`irs` when uploading). EXPERIMENTAL.
- `helixgen device rm <name> --from <setlist> [--also-pool] [--cid N] [--yes] [--json]` — **drop a setlist's reference** to a preset. The pool preset survives — and stays available to every other setlist referencing it — unless `--also-pool`. **Never orphans:** with another setlist still referencing it, `--also-pool` fails and names the holders instead of deleting (the setlist reference you asked for is still dropped). Locks `library`. EXPERIMENTAL.
- `helixgen device move <name> --in <setlist> --to <N> [--cid N] [--json]` — **reposition a preset within a setlist's order**, directly on the device. Takes effect immediately; nothing local to update afterwards. Locks `library`. EXPERIMENTAL.

**Identity is the display name, and ambiguity is an error.** Device preset names are not guaranteed unique. These verbs match within the target setlist first, then the pool; when a name matches more than one preset the verb **fails and prints the competing cids** rather than guessing. Pass `--cid` to disambiguate, or rename one (`device rename`).

### Backup / diff / restore (the device photograph)

- `helixgen device backup [--dir <root>] [--dry-run] [--json]` — **photograph the device** into `<root>/<serial>/`: `pool/<Name>.sbe` (the device's own content bytes), `setlists/<Name>.json` (an ordered array of preset names — this is where order is recorded), `irs/<irhash>.wav` + `irs/index.json`, and `device.json`. Default root `$HELIXGEN_HOME/backup` (override `$HELIXGEN_BACKUP`). **Read-only and non-activating** (`get_content`, never `load_preset`), no lease. Unchanged files are not rewritten, so a re-run leaves a clean `git status`; presets gone from the device are pruned from the tree — **except after any failed read**, when nothing is pruned (a preset that could not be fetched must never look deleted). **`--dry-run` is the diff against the live device**: it reports exactly what would change and writes nothing, which is why there is no separate `diff` verb. It wires a git textconv (`*.sbe diff=helixgen-sbe` → `device decode`) so the photograph diffs as readable JSON. EXPERIMENTAL.
- `helixgen device restore [--dir <root>] [--serial S] [--from <git-ref>] [--setlist <name>] [--prune] [--dry-run] [--yes] [--json]` — **replay a photograph onto the hardware**: pool content first, then setlist order. Additive-and-update by default — it creates what is missing and updates what differs, and never deletes. `--from HEAD~3` materialises the snapshot as of a git ref (the reason the tree is git-tracked) without disturbing the working tree. Restores from the device's own bytes, so **no transcoder is in the path**. **`--prune` also DELETES device presets absent from the snapshot** — the one genuinely destructive path here, with no undo on the device; always `--dry-run` first and confirm. Locks `library`+`irs` (a `--dry-run` takes no lease). EXPERIMENTAL.
- `helixgen device decode <file.sbe|-> [--indent N]` — **print a device content blob as native JSON**. Wholly OFFLINE. Lossless: it renders the device's own structure (4CC keys) with no transcoder, so nothing is dropped the way `device to-hsp` drops Command Center commands and MIDI CC bindings. This is the textconv driver `device backup` wires up. Reads stdin when SOURCE is `-`.

**Why `.sbe` and not `.hsp`.** `.hsp` is a lossy *view* of device content in the backwards direction only: `device to-hsp` drops Command Center commands (#16) and MIDI CC controller bindings (#33), and reports other uncarryable details one line per loss on stderr. A backup built from `.hsp` would therefore silently ship a preset whose Command Center assignments were gone, and restore would put that damaged version back. Forward (`.hsp` → device) is not lossy at all.

**IR collection is cross-checked.** A backup collects the union of the device's `-11` IR listing and every `irhash` actually referenced by a pulled preset, then verifies each pulled file's MD5 equals its hash. The listing cache is known to go stale (backlog #38 — an IR imported by another client can stay unlisted for 11+ minutes), so listing alone would silently under-collect. Anything unpullable or mismatched is reported in `irs.missing[]` with a reason, never silently dropped. Because the device's IRs are the processed 8192-sample form (~32KB), a backup is **self-sufficient**: restore does not depend on the user still holding the source WAVs.

### Migrating off the retired manifest

`~/.helixgen/setlists/manifest.json` and `device copy` are **gone**, along with `device add`, `device unsync`, `device library`, `device slots`, `device install`, `helixgen register`, and `device setlist add`/`remove`/`create-local`/`sync-on`/`sync-off`. `device copy` replaces `install`; `device rm` replaces `delete <cid> --setlist`; `device move` replaces the manifest-mediated reorder.

The device is the truth, so there is nothing to convert. If a legacy manifest is present, the migration is three steps:

1. `helixgen device backup` — photograph the live device into the new tree.
2. Commit it (`git -C ~/.helixgen add -A backup && git -C ~/.helixgen commit -m 'device: initial photograph'`).
3. Delete `~/.helixgen/setlists/manifest.json`.

`~/.helixgen/device-backups/` (the old flat `NN-slot-Name.sbe` files) is left alone as history.