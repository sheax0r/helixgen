...
- `helixgen show-block "<name>"` — print a block's exact param names, types, **real min/max ranges, units, display scaling and enum labels** (engine 0.45.0; sourced from the device model definitions, not from a single sighted preset). **Run this before writing a spec** — param names are case-sensitive, the generator rejects unknown ones, and the unit matters: an amp's `Level` is decibels (`float -40..10 dB (default -10)`) while `ChVol` is a `0..1` knob, so writing `0.5` on the former is +10.5 dB, not half volume. A single previously-sighted value is still shown, labelled `sighted`.
