...
- `helixgen device rm <name> --from <setlist> [--also-pool] [--cid N] [--yes] [--json]` — **drop a setlist's reference** to a preset. The pool preset survives — and stays available to every other setlist referencing it — unless `--also-pool`. Never orphans. Locks `library`. EXPERIMENTAL.
